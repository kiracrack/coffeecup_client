...
Public Class frmTransferRequestSelectParticular
    ...
    Public Sub ViewIventoryItem()
        ...
        msda = New MySqlDataAdapter("Select PRODUCTID as 'Item Code', " _
                         + " ITEMNAME as 'Particulars', Unit " _
                         + " from tblglobalproducts inner join tblprocategory on tblglobalproducts.catid = tblprocategory.catid  where consumable=1 and   " _
                         + " (ITEMNAME like '%" & textsearch.Text & "%' or PRODUCTID like '%" & textsearch.Text & "%' or (select DESCRIPTION from tblprocategory where CATID = tblglobalproducts.CATID) like '%" & textsearch.Text & "%' ) " _
                         + " order by ITEMNAME asc", conn)


        msda.Fill(dst, 0)
        MyDataGridView.DataSource = dst.Tables(0)
        GridHideColumn(MyDataGridView, {"Item Code"})
        GridColumnAlignment(MyDataGridView, {"Item Code", "Available Quantity", "Unit"}, DataGridViewContentAlignment.MiddleCenter)
    ...
    Public Sub showInfo()
        ...
        frmTransferQuantitySelect.mode.Text = "request"
