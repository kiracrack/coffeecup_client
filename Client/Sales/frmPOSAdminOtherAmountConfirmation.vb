...
Imports System
Public Class frmPOSAdminOtherAmountConfirmation
    ...
    Public Function UserConfirmation() As Boolean
        ...
        If GlobalStrictadminconfirmed = True Then
            com.CommandText = "SELECT * from tblaccounts where username='" & rchar(UCase(txtusername.Text)) & "'  and password='" & EncryptTripleDES(UCase(txtusername.Text) + txtpassword.Text) & "' and (coffeecupposition = (select authcode from tbluserauthority where officeapprover=1 or corpapprover=1) or lcase(username) = 'root')" : rst = com.ExecuteReader()
        Else
