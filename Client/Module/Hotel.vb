...
Module Hotel
    ...
    Public Function GetRoomFolioInfo(ByVal roomfolioid As String, ByVal column As String) As String
        com.CommandText = "select * from tblhotelfolioroom where folioid='" & roomfolioid & "'" : rst = com.ExecuteReader()
        While rst.Read
            roomfolioid = rst(column).ToString
        End While
        rst.Close()
        Return roomfolioid
    End Function
    ...
    Public Sub UpdateFolioSummary(ByVal bookingno As String)
        ...
        com.CommandText = "drop temporary table if exists tmpeditedroomfolio" : com.ExecuteNonQuery()
        com.CommandText = "CREATE temporary table if not exists tmpeditedroomfolio ( `datetrn` DATE NOT NULL,`rateid` VARCHAR(15) NOT NULL,`roomid` VARCHAR(15) NOT NULL, `pax` INTEGER UNSIGNED NOT NULL,  `child` INTEGER UNSIGNED NOT NULL,  `extra` INTEGER UNSIGNED NOT NULL)ENGINE = InnoDB;" : com.ExecuteNonQuery()
        com.CommandText = "insert into tmpeditedroomfolio select datetrn,rateid,roomid,adult,child,extra from tblhotelfolioroomsummary where roomid<>'' and foliono='" & bookingno & "' and (folioid not in (select folioid from tblhotelfolioroom where foliono='" & bookingno & "') or folioid in (select folioid from tblhotelfolioroom where foliono='" & bookingno & "'))" : com.ExecuteNonQuery()
        ...
        For nt = 0 To st.Tables(0).Rows.Count - 1
            ...
            Dim nX As Int32 = 0
            For x = 0 To nights - 1
                Dim OriginalRate As String = st.Tables(0).Rows(nt)("rateid").ToString()

                Dim nPax As Integer = Val(st.Tables(0).Rows(nt)("pax").ToString())
                Dim nChild As Integer = Val(st.Tables(0).Rows(nt)("child").ToString())
                Dim nExtra As Integer = Val(st.Tables(0).Rows(nt)("extra").ToString())
                Dim nRateid As String = st.Tables(0).Rows(nt)("rateid").ToString()
                com.CommandText = "select * from tmpeditedroomfolio where datetrn='" & ConvertDate(datTim1.AddDays(x)) & "' and roomid='" & st.Tables(0).Rows(nt)("roomid").ToString() & "'" : rst = com.ExecuteReader
                While rst.Read
                    nPax = Val(rst("pax").ToString)
                    nChild = Val(rst("child").ToString)
                    nExtra = Val(rst("extra").ToString)
                    nRateid = rst("rateid").ToString
                    If OriginalRate <> nRateid Then
                        nX += 1
                    End If
                End While
                rst.Close()

                Dim adultRate As Double = Val(qrysingledata("ttl", " ifnull(sum(amount),0) as ttl", "tblhotelratesbreakdown where dayrate='" & If(OriginalRate = nRateid, x, nX - 1) & "' and ratecode='" & nRateid & "' and breakdowntype='roomrate'"))
                Dim childRate As Double = Val(qrysingledata("ttl", " ifnull(sum(amount),0) as ttl", "tblhotelratesbreakdown where dayrate='" & If(OriginalRate = nRateid, x, nX - 1) & "' and ratecode='" & nRateid & "' and breakdowntype='child'"))
                Dim extraRate As Double = Val(qrysingledata("ttl", " ifnull(sum(amount),0) as ttl", "tblhotelratesbreakdown where dayrate='" & If(OriginalRate = nRateid, x, nX - 1) & "' and ratecode='" & nRateid & "' and breakdowntype='extra'"))

                If adultRate > 0 Then
                    AdultDayRate = If(OriginalRate = nRateid, x, nX - 1)
                    xAdultRate = adultRate
                End If
                If childRate > 0 Then
                    ChildDayRate = If(OriginalRate = nRateid, x, nX - 1)
                    xChildRate = childRate
                End If
                If extraRate > 0 Then
                    XtraDayRate = If(OriginalRate = nRateid, x, nX - 1)
                    xExtraRate = extraRate
                End If

                Dim totalAdultRate As Double = If(CBool(st.Tables(0).Rows(nt)("chargepernight").ToString()) = True, xAdultRate, xAdultRate * nPax)
                Dim totalChildRate As Double = xChildRate * nChild
                Dim totalExtraRate As Double = xExtraRate * nExtra
                Dim totaltrn As Double = totalAdultRate + totalChildRate + totalExtraRate
                Dim roomtrncode As String = bookingno & "-" & st.Tables(0).Rows(nt)("folioid").ToString() & "-" & x
                com.CommandText = "insert into tblhotelfolioroomsummary set folioid='" & st.Tables(0).Rows(nt)("folioid").ToString() & "', foliono='" & bookingno & "', " _
                            + " hotelcif='" & st.Tables(0).Rows(nt)("hotelcif").ToString() & "', " _
                            ...
                            + " adultrate='" & xAdultRate & "', " _
                            + " child='" & nChild & "', " _
                            + " childrate='" & If(nChild = 0, 0, xChildRate) & "', " _
                            + " extra='" & nExtra & "', " _
                            + " extrarate='" & If(nExtra = 0, 0, xExtraRate) & "', " _
                            + " total='" & totaltrn & "'" : com.ExecuteNonQuery()

                If xAdultRate > 0 Then
                    com.CommandText = "insert into tblhotelfolioroombreakdown (foliono,folioid,roomid,roomtrncode,dayno,ratecode,breakdowntype,itemcode,itemname,amount) select '" & bookingno & "','" & st.Tables(0).Rows(nt)("folioid").ToString() & "','" & st.Tables(0).Rows(nt)("roomid").ToString() & "','" & roomtrncode & "'," & If(OriginalRate = nRateid, x, nX - 1) & ",ratecode,breakdowntype,itemcode,itemname,amount from tblhotelratesbreakdown where dayrate='" & AdultDayRate & "' and ratecode='" & nRateid & "' and breakdowntype='roomrate'" : com.ExecuteNonQuery()
                End If
                If xChildRate > 0 And Val(st.Tables(0).Rows(nt)("child").ToString()) > 0 Then
                    com.CommandText = "insert into tblhotelfolioroombreakdown (foliono,folioid,roomid,roomtrncode,dayno,ratecode,breakdowntype,itemcode,itemname,amount) select '" & bookingno & "','" & st.Tables(0).Rows(nt)("folioid").ToString() & "','" & st.Tables(0).Rows(nt)("roomid").ToString() & "','" & roomtrncode & "'," & If(OriginalRate = nRateid, x, nX - 1) & ",ratecode,breakdowntype,itemcode,itemname,amount from tblhotelratesbreakdown where dayrate='" & ChildDayRate & "' and ratecode='" & nRateid & "' and breakdowntype='child'" : com.ExecuteNonQuery()
                End If
                If xExtraRate > 0 And Val(st.Tables(0).Rows(nt)("extra").ToString()) > 0 Then
                    com.CommandText = "insert into tblhotelfolioroombreakdown (foliono,folioid,roomid,roomtrncode,dayno,ratecode,breakdowntype,itemcode,itemname,amount) select '" & bookingno & "','" & st.Tables(0).Rows(nt)("folioid").ToString() & "','" & st.Tables(0).Rows(nt)("roomid").ToString() & "','" & roomtrncode & "'," & If(OriginalRate = nRateid, x, nX - 1) & ",ratecode,breakdowntype,itemcode,itemname,amount from tblhotelratesbreakdown where dayrate='" & XtraDayRate & "' and ratecode='" & nRateid & "' and breakdowntype='extra'" : com.ExecuteNonQuery()
                End If
