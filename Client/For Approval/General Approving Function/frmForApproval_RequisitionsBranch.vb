...
Public Class frmForApproval_RequisitionsBranch
    ...
    Public Function loaddetails(ByVal id As String)
        com.CommandText = "select *, (select officename from tblcompoffice where officeid =tblrequisitions.officeid ) as 'office', " _
                                 ...
                                 + " (select custombranchapproval from tblcompoffice where officeid =tblrequisitions.officeid) as custom, " _
    ...
    Public Sub ShowParticularsItems()
        ...
        msda = New MySqlDataAdapter("Select trnid, (select ITEMNAME from tblglobalproducts where PRODUCTID = tblrequisitionsitem.PRODUCTID ) as 'Particular', " _
        ...
        GridHideColumn(MyDataGridView_Particular, {"trnid"})
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles cmdHold.Click
        ...
        If MessageBox.Show("Are you sure you want to put on hold on this request?", GlobalOrganizationName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
            Dim applevel As String = "" : Dim apptitle As String = "" : Dim finalapp As Boolean = False
            If ckCustom.Checked = True Then
                com.CommandText = "select * from tblapproverclientprocess where authcode='" & globalAuthcode & "' and apptype='requisition-approving-process' and requestcode='" & potypeid.Text & "' and officeid='" & officeid.Text & "' " : rst = com.ExecuteReader
            Else
                com.CommandText = "select * from tblapproverclientprocess where authcode='" & globalAuthcode & "' and apptype='requisition-approving-process' and requestcode='" & potypeid.Text & "'" : rst = com.ExecuteReader
            End If
            While rst.Read
                apptitle = rst("apptitle").ToString
                applevel = rst("applevel").ToString
                finalapp = CBool(rst("finalapp"))
            End While
            rst.Close()
            com.CommandText = "INSERT INTO `tblapprovalhistory` set approvaltype= '" & potypeid.Text & "',appdescription='requisition', mainreference='" & pid.Text & "', referenceno='" & pid.Text & "',status='hold',remarks='" & rchar(txtRemarks.Text) & "',apptitle='" & apptitle & "',applevel='" & applevel & "',confirmid='" & globaluserid & "',confirmby='" & globalfullname & "',position='" & globalposition & "',dateconfirm=current_timestamp,finalapprover=" & finalapp & ",corporateapproval=0" : com.ExecuteNonQuery()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles cmdApproved.Click
        ...
        If MessageBox.Show("Are you sure you want to approve this request?", GlobalOrganizationName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
            ...
            End While
            rst.Close()
            com.CommandText = "INSERT INTO `tblapprovalhistory` set approvaltype= '" & potypeid.Text & "',appdescription='requisition', mainreference='" & pid.Text & "', referenceno='" & pid.Text & "',status='approved',remarks='" & rchar(txtRemarks.Text) & "',apptitle='" & apptitle & "',applevel='" & applevel & "',confirmid='" & globaluserid & "',confirmby='" & globalfullname & "',position='" & globalposition & "',dateconfirm=current_timestamp,finalapprover=" & finalapp & ",corporateapproval=0" : com.ExecuteNonQuery()

            If finalapp = True Then
                com.CommandText = "update tblrequisitions set approvedbranch=true, dateapproved=current_timestamp where pid='" & pid.Text & "'" : com.ExecuteNonQuery()
            ...
            Else
                NextEmailBranchLevelRequisitionApprover(pid.Text, txtpotype.Text, txtRemarks.Text)
            End If
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles cmdDisaproved.Click
        If txtRemarks.Text = "" Or txtRemarks.Text = "Please enter message" Then
            MessageBox.Show("Please enter your message!", GlobalOrganizationName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txtRemarks.Focus()
            Exit Sub
        ...
        If MessageBox.Show("Are you sure you want to disapprove this request?", GlobalOrganizationName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
            com.CommandText = "INSERT INTO `tblapprovalhistory` (approvaltype,appdescription,mainreference,referenceno,status,remarks,apptitle,applevel,confirmid,confirmby,position,dateconfirm,finalapprover,corporateapproval) " _
                      + " SELECT '" & potypeid.Text & "','requisition','" & pid.Text & "','" & pid.Text & "','disapproved','" & rchar(txtRemarks.Text) & "',apptitle,applevel,'" & globaluserid & "','" & globalfullname & "','" & globalposition & "',current_timestamp,finalapp,0 FROM tblapproverclientprocess where authcode='" & globalAuthcode & "' and requestcode='" & potypeid.Text & "' and apptype='requisition-approving-process' " & If(ckCustom.Checked = True, " and officeid='" & officeid.Text & "'", " and officeid=''") : com.ExecuteNonQuery()
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
        PrintDatagridview("Purchase Request<br/><strong>" & pid.Text & "</strong>", "Purchase Request", "Office: " & txtoffice.Text & "<br/>Requested By: " & txtRequestby.Text & "<br/>Request Type: " & txtpotype.Text, MyDataGridView_Particular, Me)
    End Sub
